...
GIT_BIN="${WPR_GIT_BIN:-${WPM_GIT_BIN:-/usr/bin/git}}"
CURL_BIN="${WPR_CURL_BIN:-${WPM_CURL_BIN:-/usr/bin/curl}}"
BUN_BIN="${WPR_BUN_BIN:-${WPM_BUN_BIN:-${HOME}/.bun/bin/bun}}"
...
http_status() {
  "${CURL_BIN}" --max-time 10 -sS -o /dev/null -w '%{http_code}' "$1" || true
}
