...
MIN_INTERVAL_MINUTES="${WPR_INGEST_OPS_MIN_INTERVAL_MINUTES:-${WPM_INGEST_OPS_MIN_INTERVAL_MINUTES:-20}}"
...
if [ -n "${WPR_HOURLY_DISCORD_WEBHOOK:-${WPM_HOURLY_DISCORD_WEBHOOK:-}}" ] || [ -n "${INGEST_OPS_DISCORD_WEBHOOK:-}" ] || [ -n "${RSS_HEALTH_DISCORD_WEBHOOK_URL:-}" ] || [ -n "${DISCORD_WEBHOOK_URL:-}" ]; then
...
if [ "${WPR_FORCE_INGEST_OPS_REPORT:-${WPM_FORCE_INGEST_OPS_REPORT:-0}}" != "1" ] && [ -f "${LAST_SUCCESS_FILE}" ]; then
  now_epoch="$(date +%s)"
