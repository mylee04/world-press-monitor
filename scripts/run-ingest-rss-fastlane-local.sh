#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
# shellcheck disable=SC1091
source "${SCRIPT_DIR}/load-local-env.sh"
load_local_env

lane="strict"
for arg in "$@"; do
  case "${arg}" in
    --lane=strict)
      lane="strict"
      ;;
    --lane=relaxed)
      lane="relaxed"
      ;;
    *)
      echo "Unknown argument: ${arg}" >&2
      exit 1
      ;;
  esac
done

LOG_DIR="${WPR_LOG_DIR:-${WPM_LOG_DIR:-${PROJECT_ROOT}/logs}}"
TIMEZONE="${INGEST_TZ:-America/Chicago}"

case "${lane}" in
  strict)
    LOG_FILE="${LOG_DIR}/ingest-rss-fastlane-strict-local.log"
    RUNNER_COMMAND=(bun run ingest:rss-fastlane)
    ;;
  relaxed)
    LOG_FILE="${LOG_DIR}/ingest-rss-fastlane-relaxed-local.log"
    RUNNER_COMMAND=(bun run ingest:rss-fastlane:relaxed)
    ;;
  *)
    echo "Unsupported lane: ${lane}" >&2
    exit 1
    ;;
esac

mkdir -p "${LOG_DIR}"
cd "${PROJECT_ROOT}"
export TZ="${TIMEZONE}"
export PATH="${PATH}:/opt/homebrew/bin:/usr/local/bin"

: "${DATABASE_URL:=postgresql://postgres:postgres@127.0.0.1:${WPR_PG_PORT:-${WPM_PG_PORT:-5432}}/${WPR_DATABASE_NAME:-${WPM_DATABASE_NAME:-wpr}}}"

if ! DB_PING_OUTPUT="$(psql "${DATABASE_URL}" -v ON_ERROR_STOP=1 -qtAX -c 'SELECT 1' 2>&1)"; then
  printf '\n[%s] START BLOCKED: local DATABASE_URL connection check failed.\n' "$(date -u '+%Y-%m-%d %H:%M:%S %Z')" >>"${LOG_FILE}"
  printf 'psql error: %s\n' "${DB_PING_OUTPUT}" >>"${LOG_FILE}"
  exit 1
fi

{
  printf '\n[%s] Start rss fastlane lane=%s\n' "$(date -u '+%Y-%m-%d %H:%M:%S %Z')" "${lane}"
  printf 'Project: %s\n' "${PROJECT_ROOT}"
  printf 'Env file: %s\n' "${WPR_ENV_FILE_SOURCE:-${WPM_ENV_FILE_SOURCE:-inline-defaults}}"
  printf 'Runner: %s\n' "${RUNNER_COMMAND[*]}"
  "${RUNNER_COMMAND[@]}"
} >>"${LOG_FILE}" 2>&1
